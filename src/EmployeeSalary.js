...
function EmployeeSalary() {
  ...
  useEffect(() => {
    axios
      .get('/payroll/salary/getAllEmpSalary')
      .then((response) => {
  ...
  const handleGeneratePaySlip = (empId) => {
    ...
    if (selectedEmployee) {
      ...
      console.log(selectedMonth, year, requestBody);
      axios
        .post(`/payroll/viewPay?month=${selectedMonth}&year=${year}`, requestBody)
        .then((response) => {
          console.log(response.data);
          toast.success('Pay slip generated successfully.', { position: 'top-center', theme: 'colored' });
          // Handle the response and display the pay slip view
        })
        .catch((error) => {
          console.log(error);
