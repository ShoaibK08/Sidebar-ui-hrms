...
//     useEffect(() => {
//         // Fetch the leave balance  information from the server when the component mounts
//         axios.get(`/payroll/leave/getById/${69}`, {
//             headers: {
//                 'Authorization': `Bearer ${token}`
//             }
//         }).then(response => {
...
const LeaveForm = () => {
    ...
    useEffect(() => {
        axios
            ...
            .then(response => {
                console.log(response.data);
                setLeaveForm(prevState => ({ ...prevState, leave: response.data }));
                toast.success('Leave data found successfully!!', { position: 'top-center', theme: 'colored' });
